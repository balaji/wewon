...
get '/logout' do
  ...
  settings.cache.get('sprints').each do |sprint|
    ...
    settings.cache.set("transitions_#{sprint_split(sprint[:name])}", nil)
  end
...
get '/history.json' do
  content_type :json
  unless not settings.cache.get("transitions_#{session[:sprint]}").nil?
    ret = {}
    settings.cache.get("data_#{session[:sprint]}").each do |hash|
      c = Curl::Easy.new("https://#{session[:login_name]}:#{session[:password]}@www10.v1host.com/sandp/rest-1.v1/Hist/Story/#{hash[:story_id]}?sel=ChangeDate,Status.Name&sort=ChangeDate")
      c.perform
      r = []
      Nokogiri::XML(c.body_str).xpath('//History/Asset').each do |row|
        r.push({
          :change_date => row.at_xpath('Attribute[@name="ChangeDate"]/text()').to_s,
          :status_name => row.at_xpath('Attribute[@name="Status.Name"]/text()').to_s
        })
      end
      ret[hash[:story_id]] = r
    end
    settings.cache.set("transitions_#{session[:sprint]}", ret)
  end
  settings.cache.get("transitions_#{session[:sprint]}").to_json
end
...
helpers do 
  ...
  def sprint_display
    ...
    today = Date.parse(Time.now.gmtime.strftime("%Y/%m/%d"))
    if end_date >= today
      begin_date < today ? "Current Sprint. #{end_date.mjd - today.mjd} day(s) left!" : "Sprint starts in #{begin_date.mjd - today.mjd} day(s)"
    else
